    ...
    <div className="bg-blue-500 sticky top-0 z-20">
      <div className="max-w-screen-2xl mx-auto px-3 sm:px-10">
        <div className="top-bar h-16 lg:h-auto flex items-center justify-between py-4 mx-auto">
          <Link
            href="/"
            className="mr-3 lg:mr-12 xl:mr-12 hidden md:hidden lg:block text-white">
          ...
          <div className="hidden md:hidden md:items-center lg:flex xl:block absolute inset-y-0 right-0 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            ...
            >
              <span className="absolute z-10 top-0 right-0 inline-flex items-center justify-center p-1 h-5 w-5 text-xs font-medium leading-none text-black transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-full">
                {totalItems}
              </span>
            ...
            <button className="pl-5 text-white text-2xl font-bold" aria-label="Login">
              {imageUrl || userInfo?.image ? (
                ...
                <Link
                  href="/user/dashboard"
                  className="leading-none font-bold font-serif block uppercase">
                  {userInfo?.name[0]}
                </Link>
              ) : (
